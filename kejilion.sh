...
case $choice in
  ...
  while true; do
    ...
    case $sub_choice in
      ...
      sudo docker exec nginx chmod -R 777 /home/web/html/$yuming/wordpress && docker exec php chmod -R 777 /home/web/html/$yuming/wordpress && docker exec php74 chmod -R 777 /home/web/html/$yuming/wordpress
      ...
      docker exec nginx chmod -R 777 /var/www/html && docker exec php chmod -R 777 /var/www/html && docker exec php74 chmod -R 777 /var/www/html
      #(删除上一行，使用以下代码，解决VPS重启后不能访问网站的问题，以下有这句代码的项都要替换。sudo docker exec nginx chmod -R 777 /home/web/html/$yuming/wordpress && docker exec php chmod -R 777 /home/web/html/$yuming/wordpress && docker exec php74 chmod -R 777 /home/web/html/$yuming/wordpress)
      dbrootpasswd=$(grep -oP 'MYSQL_ROOT_PASSWORD:\s*\K.*' /home/web/docker-compose.yml | tr -d '[:space:]')
      dbuse=$(grep -oP 'MYSQL_USER:\s*\K.*' /home/web/docker-compose.yml | tr -d '[:space:]')
      dbusepasswd=$(grep -oP 'MYSQL_PASSWORD:\s*\K.*' /home/web/docker-compose.yml | tr -d '[:space:]')
      docker exec mysql mysql -u root -p"$dbrootpasswd" -e "CREATE DATABASE $dbname; GRANT ALL PRIVILEGES ON $dbname.* TO \"$dbuse\"@\"%\";"
