...
case $choice in
  ...
  while true; do
    ...
    case $sub_choice in
      ...
      cd ~
      curl https://get.acme.sh | sh
      ~/.acme.sh/acme.sh --register-account -m fandianhuai@gmail.com --issue -d $yuming --standalone --key-file "$base_dir"/cert/key.pem --cert-file "$base_dir"/cert/cert.pem --force
      docker start nginx
